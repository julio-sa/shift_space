...
@pytest.fixture
def restore_backend():
    """Restore the plotting api to matplotlib"""
    with pandas.option_context("plotting.api", "matplotlib"):
        yield


def test_backend_is_not_module():
    msg = "Could not find plotting api 'not_an_existing_module'."
    with pytest.raises(ValueError, match=msg):
        pandas.set_option("plotting.api", "not_an_existing_module")
...
def test_backend_is_correct(monkeypatch, restore_backend, dummy_backend):
    ...
    pandas.set_option("plotting.api", "pandas_dummy_backend")
    assert pandas.get_option("plotting.api") == "pandas_dummy_backend"
    assert (
...
def test_backend_can_be_set_in_plot_call(monkeypatch, restore_backend, dummy_backend):
    ...
    assert pandas.get_option("plotting.api") == "matplotlib"
...
def test_register_entrypoint(restore_backend, tmp_path, monkeypatch, dummy_backend):
    ...
    # fall back to api lookup by module name
    ...
    with pandas.option_context("plotting.api", "my_ep_backend"):
...
def test_setting_backend_without_plot_raises(monkeypatch):
    ...
    assert pandas.options.plotting.backend == "matplotlib"
    with pytest.raises(
        ValueError, match="Could not find plotting api 'pandas_plot_backend'."
    ):
        pandas.set_option("plotting.api", "pandas_plot_backend")
...
def test_no_matplotlib_ok():
    msg = (
        'matplotlib is required for plotting when the default api "matplotlib" is '
        "selected."
    )
