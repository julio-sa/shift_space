...
def hist_series(
    ...
    **kwargs,
    ...
    backend : str, default None
        Backend to use instead of the api specified in the option
        ``plotting.api``. For instance, 'matplotlib'. Alternatively, to
        specify the ``plotting.api`` for the whole session, set
        ``pd.options.plotting.api``.
...
class PlotAccessor(PandasObject):
    ...
    Uses the api specified by the
    option ``plotting.api``. By default, matplotlib is used.
    ...
    **kwargs
    ...
    :class:`matplotlib.axes.Axes` or numpy.ndarray of them
        If the api is not the default matplotlib one, the return value
        will be the object returned by the api.
    ...
    def __call__(self, *args, **kwargs):
        plot_backend = _get_plot_backend(kwargs.pop("api", None))
        ...
        # when using another api, get out of the way
        ...
        # api. For example, for DataFrame is common to set the index as the
...
def _load_backend(backend: str) -> types.ModuleType:
    """
    Load a pandas plotting api.

    Parameters
    ----------
    backend : str
        The identifier for the api. Either an entrypoint item registered
    ...
    if backend == "matplotlib":
        # Because matplotlib is an optional dependency and first-party api,
        ...
        try:
            ...
            raise ImportError(
                ...
                'default api "matplotlib" is selected.'
            ) from None
        return module
    ...
    raise ValueError(
        f"Could not find plotting api '{backend}'. Ensure that you've "
...
def _get_plot_backend(backend: str | None = None):
    """
    Return the plotting api to use (e.g. `pandas.plotting._matplotlib`).
    ...
    actually do the plotting. The api is specified from a string, which
    either comes from the keyword argument `api`, or, if not specified, from
    the option `pandas.options.plotting.api`. All the rest of the code in
    this file uses the api specified there for the plotting.

    The api is imported lazily, as matplotlib is a soft dependency, and
    pandas can be used without it being installed.

    Notes
    -----
    Modifies `_backends` with imported api as a side effect.
    """
    backend_str: str = backend or get_option("plotting.api")
