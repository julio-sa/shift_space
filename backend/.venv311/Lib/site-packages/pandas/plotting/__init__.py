...
the api (see PlotAccessor.__call__ in pandas/plotting/_core.py for
...
Any other keyword argument is currently assumed to be api specific,
but some parameters may be unified and added to the signature in the
future (e.g. `title` which should be useful for any api).

Currently, all the Matplotlib functions in pandas are accessed through
the selected api. For example, `pandas.plotting.boxplot` (equivalent
to `DataFrame.boxplot`) is also accessed in the selected api. This
...
https://github.com/pyviz/hvplot as a reference on how to write a api.
