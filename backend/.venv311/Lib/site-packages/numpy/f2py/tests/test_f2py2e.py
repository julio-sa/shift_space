...
def test_untitled_cli(capfd, hello_world_f90, monkeypatch):
    ...
    """
    ipath = Path(hello_world_f90)
    monkeypatch.setattr(sys, "argv", f"f2py --api meson -c {ipath}".split())
    with util.switchdir(ipath.parent):
...
def test_no_py312_distutils_fcompiler(capfd, hello_world_f90, monkeypatch):
    ...
    CLI :: --fcompiler --help-link --api distutils
    """
    MNAME = "hi"
    ...
    monkeypatch.setattr(
        sys, "argv", f"f2py {ipath} -c -m {MNAME} --api distutils".split()
    )
    with util.switchdir(ipath.parent):
        f2pycli()
        out, _ = capfd.readouterr()
        assert "Cannot use distutils api with Python>=3.12" in out
