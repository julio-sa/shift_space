...
build api options (only effective with -c)
[NO_MESON] is used to indicate an option not meant to be used
with the meson api or above Python 3.12:
  ...
  --api            <backend_type>
                       Specify the build api for the compilation process.
...
def make_f2py_compile_parser():
    ...
    parser.add_argument("--api", choices=['meson', 'distutils'], default='distutils')
...
def preparse_sysargv():
    ...
    if MESON_ONLY_VER and backend_key == 'distutils':
        outmess("Cannot use distutils api with Python>=3.12,"
                " using meson api instead.\n")
        backend_key = "meson"
