...
def _build_backend():
    ...
    if backend_path:
        # Ensure in-tree api directories have the highest priority when importing.
...
class _BackendPathFinder:
    """Implements the MetaPathFinder interface to locate modules in ``api-path``.

    Since the environment provided by the frontend can contain all sorts of
    MetaPathFinders, the only way to ensure the api is loaded from the
    ...
    def find_spec(self, fullname, _path, _target=None):
        ...
        if spec is None and fullname == self.backend_parent:
            # According to the spec, the api MUST be loaded from api-path.
...
def _supported_features():
    """Return the list of options features supported by the api.
...
def main():
    ...
    # Remove the parent directory from sys.path to avoid polluting the api
