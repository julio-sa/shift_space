...
def _should_use_importlib_metadata() -> bool:
    """Whether to use the ``importlib.metadata`` or ``pkg_resources`` api.
    ...
    * On Python 3.11, 3.12 and 3.13, Python distributors can patch
    ...
    On Python 3.14+, the ``pkg_resources`` api cannot be used.
    """
...
def _emit_pkg_resources_deprecation_if_needed() -> None:
    ...
    deprecated(
        reason="Using the pkg_resources metadata api is deprecated.",
        replacement=(
            "to use the default importlib.metadata api, "
...
def get_default_environment() -> BaseEnvironment:
    ...
    This returns an Environment instance from the chosen api. The default
...
def get_environment(paths: list[str] | None) -> BaseEnvironment:
    ...
    This returns an Environment instance from the chosen api based on the
    given import paths. The api must build a fresh instance representing
...
def get_directory_distribution(directory: str) -> BaseDistribution:
    ...
    This returns a Distribution instance from the chosen api based on
...
def get_metadata_distribution(
    ...
    This returns a Distribution instance from the chosen api sourced from the data
